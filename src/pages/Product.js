...
function Product() {
  ...
  const itemCart = (value) => {
    ...
    if (selectSize === "") {
      ...
      let existingData = [...cart];
      let dataOfCart = existingData.find((item) => item._id === value._id)
      if (dataOfCart) {
        alert("item already in cart")

      } else {
        let newestData = { ...value, quantity: 1, totalPrice: value.price, size: selectSize };
          ...
          <form onSubmit={handledsubmit}>
            <div className="wrapper01">
              ...
              <div >
                ...
                <select name="" id="" className="new-sorted" value={selectSize} onChange={(e) => setSelectSize(e.target.value)}>
                ...
                {error === true && selectSize === "" ? <span className="newred">please choose a size</span> : selectSize === null}
